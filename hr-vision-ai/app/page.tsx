...
export default function Home() {
  ...
  const [messages, setMessages] = useState([
    {
      role: 'ai',
      text: '你好！我是你的 HRBP 14天躍升教練。在對話中，系統將依據您的年資深度自適應推進，拆解業務偽需求並產出主管可簽核之落地方案。\n\n請在下方沙盒輸入您面臨的具體組織難題，或直接點選標籤開始診斷！'
    }
  ]);
  ...
  const submitBooking = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        window.location.href = `mailto:mshappyj@gmail.com?subject=諮詢預約&body=${encodeURIComponent(formData.challenge)}`;
      }
    } catch {
      window.location.href = `mailto:mshappyj@gmail.com?subject=諮詢預約&body=${encodeURIComponent(formData.challenge)}`;
    } finally {
    ...
    <div style={{ backgroundColor: '#fcfcfd', minHeight: '100vh', color: '#1e293b', fontFamily: '-apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif' }}>
      ...
      <div style={{ maxWidth: '1240px', margin: '0 auto', padding: '0 1rem', display: 'grid', gridTemplateColumns: '1fr 380px', gap: '2rem' }}>
        <main style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
          <section id="diagnostics" style={{ background: '#fff', border: '1px solid #e2e8f0', borderRadius: '16px', padding: '2.5rem 2rem', textAlign: 'center', boxShadow: '0 2px 8px rgba(0,0,0,0.03)' }}>
            ...
            <div style={{ display: 'flex', maxWidth: '720px', margin: '0 auto', background: '#f8fafc', borderRadius: '50px', padding: '0.4rem 0.5rem 0.4rem 1.25rem', border: '1px solid #e2e8f0' }}>
              ...
              <button 
                ...
                style={{ background: sandboxInput.trim() ? '#2563eb' : '#cbd5e1', color: '#fff', border: 'none', padding: '0.65rem 1.5rem', borderRadius: '50px', fontWeight: 600, cursor: sandboxInput.trim() ? 'pointer' : 'not-allowed' }}>
                開始診斷
              </button>
            </div>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'center', gap: '0.5rem', maxWidth: '750px', margin: '0 auto' }}>
              {quickTags.map((item, idx) => (
                <button 
                  ...
                  style={{ background: '#fff', border: '1px solid #e2e8f0', padding: '0.4rem 0.85rem', borderRadius: '50px', fontSize: '0.825rem', color: '#475569', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '0.4rem' }}>
          ...
          <article style={{ background: '#fff', border: '1px solid #e2e8f0', borderRadius: '16px', padding: '2rem' }}>
            ...
            <ul style={{ paddingLeft: '1.25rem', fontSize: '0.85rem', color: '#475569', lineHeight: 1.6 }}>
              ...
              <li><strong>三層行動閉狂：</strong>提供包含「短線降火、中線賦能、長線機制」的完整架構。</li>
            </ul>
          </article>
        </main>

        <aside style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
          <div id="booking" style={{ background: '#fff', border: '1px solid #e2e8f0', borderRadius: '16px', padding: '1.75rem', position: 'sticky', top: '5.5rem' }}>
            ...
            <form onSubmit={submitBooking} style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.5rem' }}>
                <input 
                  ...
                  style={{ width: '100%', padding: '0.65rem 0.75rem', border: '1px solid #e2e8f0', borderRadius: '8px', background: '#f8fafc', outline: 'none', fontSize: '0.9rem' }}
                />
                <input 
                  type="email" 
                  placeholder="公司信箱" 
              ...
              <textarea 
                ...
                style={{ width: '100%', height: '110px', padding: '0.75rem', border: '1px solid #e2e8f0', borderRadius: '8px', background: '#f8fafc', outline: 'none', fontSize: '0.85rem', resize: 'vertical' }}
              />
              <button 
                type="submit"
                disabled={isSubmitting}
                style={{ width: '100%', background: isSubmitting ? '#94a3b8' : '#2563eb', color: '#fff', border: 'none', padding: '0.8rem', borderRadius: '8px', fontWeight: 700, fontSize: '0.95rem', cursor: isSubmitting ? 'not-allowed' : 'pointer' }}>
      ...
      {showKeyModal && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
          <div style={{ background: '#fff', borderRadius: '12px', padding: '1.5rem', width: '90%', maxWidth: '420px' }}>
            ...
            <input 
              ...
              style={{ width: '100%', padding: '0.65rem', border: '1px solid #cbd5e1', borderRadius: '6px', fontSize: '0.85rem', marginBottom: '1rem' }}
            />
            <div style={{ display: 'flex', gap: '0.5rem' }}>
